...
// A partir de aquÁ $conexion (PDO) está disponible
...
function saveConfig(PDO $pdo): void
{
    ...
    $uploadBase  = __DIR__ . '/../../public/uploads/config';
    ...
    foreach ($imageFields as $key => $subdir) {
        ...
        if (move_uploaded_file($file['tmp_name'], $dest)) {
            $updates[] = ['clave' => $key, 'valor' => 'uploads/config/' . $subdir . '/' . basename($dest), 'es_secreto' => 0];
        }
    }
