...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "registro") {
        ...
        if (!in_array($rol, $roles_validos)) {
            header("Location: /ascc/views/auth/registro.php?error=rol_invalido");
            exit;
        }

        // Validar que las contraseñas coincidan
        if ($password !== $password2) {
            header("Location: /ascc/views/auth/registro.php?error=passwords_no_coinciden");
            exit;
        }
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                header("Location: /ascc/views/auth/registro.php?error=email_existe");
                exit;
            }
            ...
            if ($stmt->rowCount() > 0) {
                header("Location: /ascc/views/auth/registro.php?error=cedula_existe");
                exit;
            }
            ...
            if (!$email_enviado) {
                ...
                header("Location: /ascc/views/auth/registro.php?error=email_no_enviado");
                exit;
            }
    ...
    if ($accion === "login") {
        ...
        try {
            ...
            if ($stmt->rowCount() === 0) {
                header("Location: /ascc/views/auth/login.php?error=credenciales_invalidas");
                exit;
            }
            ...
            if ($usuario["estado"] === "bloqueado") {
                header("Location: /ascc/views/auth/login.php?error=cuenta_bloqueada");
                exit;
            }
