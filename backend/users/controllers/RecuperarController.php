...
switch ($accion) {
    ...
    default:
        header("Location: /ascc/frontend/users/views/auth/recuperar.php");
        exit;
}
...
function solicitarRecuperacion($conexion)
{
    ...
    if (empty($email)) {
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=email_vacio");
        exit;
    }
    ...
    if ($stmt->rowCount() === 0) {
        ...
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?success=1");
        exit;
    }
    ...
    try {
        ...
        if ($stmt->rowCount() === 0) {
            ...
            header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=tabla_no_existe");
            exit;
        }
    } catch (PDOException $e) {
        error_log("Error al verificar tabla: " . $e->getMessage());
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=sistema");
        exit;
    }
    ...
    try {
        ...
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=bd_error");
        exit;
    }
...
function restablecerContrasena($conexion)
{
    ...
    if (empty($token) || empty($password) || empty($password2)) {
        header("Location: /ascc/frontend/users/views/auth/restablecer.php?token=$token&error=campos_vacios");
        exit;
    }

    if ($password !== $password2) {
        header("Location: /ascc/frontend/users/views/auth/restablecer.php?token=$token&error=passwords_no_coinciden");
        exit;
    }

    if (strlen($password) < 6) {
        header("Location: /ascc/frontend/users/views/auth/restablecer.php?token=$token&error=password_corta");
        exit;
    }
    ...
    if ($stmt->rowCount() === 0) {
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=token_invalido");
        exit;
    }
    ...
    if (strtotime($reset['expiracion']) < time()) {
        header("Location: /ascc/frontend/users/views/auth/recuperar.php?error=token_expirado");
        exit;
    }
    ...
    try {
        ...
        header("Location: /ascc/frontend/users/views/auth/login.php?password_changed=1");
        exit;
    } catch (PDOException $e) {
        $conexion->rollBack();
        error_log("Error al restablecer contraseña: " . $e->getMessage());
        header("Location: /ascc/frontend/users/views/auth/restablecer.php?token=$token&error=sistema");
        exit;
    }
}
