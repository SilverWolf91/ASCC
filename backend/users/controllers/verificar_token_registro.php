...
if (!isset($_SESSION['pending_registro'])) {
    header('Location: /ascc/frontend/users/views/auth/registro.php?error=sesion_expirada');
    exit;
}
...
if (time() > ($pending['expiry'] ?? 0)) {
    ...
    header('Location: /ascc/frontend/users/views/auth/registro.php?error=token_expirado');
    exit;
}
...
if (($pending['intentos'] ?? 0) >= 3) {
    unset($_SESSION['pending_registro']);
    header('Location: /ascc/frontend/users/views/auth/registro.php?error=demasiados_intentos');
    exit;
}
...
if (!hash_equals($pending['token'], $token_ingresado)) {
    ...
    header('Location: /ascc/frontend/users/views/auth/verificar_registro.php?error=token_invalido&restantes=' . $intentos_restantes);
    exit;
}
...
try {
    ...
    header('Location: /ascc/frontend/users/views/auth/verificar_registro.php?error=error_servidor');
    exit;
}
